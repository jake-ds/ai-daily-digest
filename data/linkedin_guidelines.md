...
### 1. 테제(Thesis) 주도
모든 포스트는 한 문장으로 요약할 수 있는 핵심 주장이 있어야 합니다.
- 좋은 예: "에이전트 시대에 살아남는 소프트웨어의 조건이 3가지로 수렴했습니다"
- 나쁜 예: "최근 AI 업계에서 여러 가지 움직임이 있었습니다" (테제 없음, 뉴스 요약에 불과)

### 2. 문화적 훅
업계에서 통용되는 격언이나 유명 문구를 한 단어만 바꿔 인지적 마찰을 생성합니다.
- 예: YC의 "Make something people want" → "Make something agents want"
- 효과: 독자가 이미 아는 맥락을 활용해 새로운 관점을 즉시 전달

### 3. 점진적 논증
각 포인트가 이전 포인트 위에 쌓여야 합니다. 병렬 나열(첫째, 둘째, 셋째)은 기계적입니다.
- 좋은 예: 문서(쉬움) → harness(어려움) → 도메인(불가능) — 난이도가 올라감
- 나쁜 예: "첫째 A, 둘째 B, 셋째 C" — 순서를 바꿔도 의미가 같으면 병렬 나열

### 4. 구체적 대비
승자 vs 패자를 이름과 숫자로 보여줍니다. 추상적 비교가 아닌 구체적 대비.
- 예: "Supabase는 문서를 MCP로 열었고, SendGrid는 닫았습니다"
- 예: "2시간 걸리던 작업이 3분으로 줄었습니다"
- 예: "764개 PR이 올라왔습니다"

### 5. 원문 소재 활용
원문 콘텐츠에서 구체적 수치, 직접 인용구, 대비 사례를 반드시 추출하여 사용합니다.
표면적 요약 대신, 원문의 '살아있는 디테일'을 활용하세요.

### 6. 종합 마무리
전체 논증의 흐름을 한 문장으로 응축합니다.
- 예: "코드에서 문서로, 문서에서 harness로, harness에서 도메인으로."
- 효과: 독자가 전체 글을 한 줄로 기억
...
**본문 구조**:
1. 테제 선언 (핵심 주장 한 문장)
2. 점진적 논증 (각 포인트가 이전 포인트 위에 쌓임, 난이도/깊이 상승)
...
YC의 유명한 격언입니다. 그런데 2025년에는 한 단어를 바꿔야 합니다.

"Make something agents want."

지난 주 Anthropic의 MCP(Model Context Protocol) 도입 이후 벌어진 일들을 보면, 에이전트 시대에 살아남는 소프트웨어의 조건이 3가지로 수렴했습니다.

1. 문서를 여는 회사가 이깁니다.

Supabase는 MCP 서버를 공식 출시했고, 일주일 만에 764개 PR이 올라왔습니다.
SendGrid는 아직 REST API 문서만 제공합니다.

차이가 뭘까요.
Supabase의 데이터베이스는 에이전트가 "스스로 연결"할 수 있습니다.
SendGrid의 이메일 API는 개발자가 "직접 연결해줘야" 합니다.

에이전트 관점에서 Supabase는 문이 열려 있고, SendGrid는 닫혀 있습니다.

2. 코드보다 harness가 중요해집니다.

여기서 한 단계 더 깊이 들어가면, 코드 자체의 가치가 줄어들고 있습니다.

Replit의 CEO Amjad Masad가 최근 이런 말을 했습니다.
"코드의 가치는 0으로 수렴한다. 남는 건 taste와 judgment다."

실제로 Cursor 사용자 데이터를 보면, 생성된 코드의 40%가 24시간 내에 수정됩니다.
코드는 일회용이 되고, 그 코드를 감싸는 테스트와 검증 체계, 즉 harness가 핵심이 됩니다.

에이전트가 코드를 쓰는 시대에, 사람이 해야 할 일은 "어떤 코드가 좋은 코드인지" 판단하는 기준을 만드는 것입니다.

3. 결국 도메인 지식이 해자가 됩니다.

문서를 열고, harness를 갖춘 다음에 남는 건 뭘까요.
"이 업계에서 진짜 중요한 게 뭔지 아는 것"입니다.

에이전트는 코드를 쓸 수 있지만, 의료 규제를 이해하지 못합니다.
에이전트는 데이터를 분석할 수 있지만, 고객이 진짜 원하는 게 뭔지 모릅니다.

ㅡ

코드에서 문서로, 문서에서 harness로, harness에서 도메인으로.

소프트웨어의 가치가 이동하고 있습니다.
저도 지금 만들고 있는 제품의 해자를 다시 점검하고 있습니다.
